...
console.log(byRole.editor.length); // 1
    ...
    async: {
        ...
        intro: "Синхронный код получает результат до перехода к следующей операции. Асинхронный код запускает долгую операцию, продолжает работу и получает её результат позже.",
        bullets: [
            "Асинхронность не означает блокировку повторного клика: состояние loading и disabled управляются отдельно.",
            "`setTimeout(callback, 0)` планирует callback на потом: очередь обрабатывается после завершения текущего синхронного кода.",
            "Promise хранит состояние будущего результата: pending, затем fulfilled со значением или rejected с ошибкой.",
            "`await` приостанавливает продолжение только текущей `async`-функции, а не весь интерфейс и не весь JavaScript.",
        ...
        tableRows: [
            {left: "sync", middle: "результат нужен сейчас", right: "обычные вычисления и преобразования"},
            {left: "async", middle: "результат появится позже", right: "HTTP, таймеры, пользовательские события"},
